...
export type RemoveCallback = (id: string) => Promise<void>;
  ...
  const handleUpload = useCallback(
    async (files: File[]) => {
      ...
      for (let i = 0; i < validFiles.length; i++) {
        ...
        try {
          const result = await onUpload(file, (progress: number) => {
            setUploads((prev) => {
              const updated = [...prev];
    ...
    [onUpload, attachments, onAttachmentsChange, uploads.length],
  );
