...
export function AssignStudentDialog({
  ...
  const handleAssign = async () => {
    ...
    try {
      // Enroll each student sequentially or in parallel
      // The API only supports single enrollment according to the user
      await Promise.all(
      ...
      <DialogContent className='sm:max-w-[500px] gap-0 p-0 overflow-hidden'>
        <div className='p-6 border-b'>
          <DialogHeader>
            ...
            <DialogDescription>
              Enroll new students into <strong>{courseTitle}</strong>.
            </DialogDescription>
          </DialogHeader>
        </div>
