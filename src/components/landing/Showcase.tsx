...
function ShowcaseSection({
  ...
  return (
    <section
      id={reverse ? undefined : "showcase"}
      className={`relative py-24 md:py-32 px-4 sm:px-6 overflow-hidden ${bg ? "bg-muted/20" : ""}`}
    >
      <div className="max-w-7xl mx-auto">
        <div
          className={`grid lg:grid-cols-2 gap-16 items-center ${reverse ? "" : ""}`}
        >
          {/* Content side */}
          <motion.div
          ...
          >
            <div
              className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full border mb-6 ${pillColor}`}
            >
            ...
            <div className="space-y-4">
              {bullets.map((b) => (
                <div key={b.label} className="flex items-start gap-3">
                  <div
                    className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${bulletColor}`}
                  >
                    <IconCheck size={12} />
                  </div>
                  <div>
      ...
      <div className="rounded-2xl border border-border/40 bg-card shadow-2xl shadow-black/10 overflow-hidden">
        ...
        <div className="p-4 space-y-3">
          {classrooms.map((c) => (
            <div
              key={c.code}
              className="rounded-xl border border-border/30 p-4 hover:border-primary/20 hover:shadow-sm transition-all cursor-pointer"
            >
              <div className="flex items-start gap-3">
                <div
                  className={`w-10 h-10 rounded-xl bg-gradient-to-br ${c.color} flex items-center justify-center flex-shrink-0`}
                >
      ...
      <div className="rounded-2xl border border-border/40 bg-card shadow-2xl shadow-black/10 overflow-hidden">
        ...
        <div className="divide-y divide-border/20">
          {notices.map((n) => (
            ...
            >
              <div
                className={`mt-0.5 p-2 rounded-lg border flex-shrink-0 ${n.color}`}
              >
                <n.icon size={14} />
              </div>
              <div className="flex-1 min-w-0">
...
function ResourcesVisual() {
  const files = [
    {
      name: "Week 5 - Lecture Notes.pdf",
      size: "2.4 MB",
      icon: IconFileText,
      color: "text-red-500 bg-red-500/10",
    },
    {
      name: "Assignment 3 - Guidelines.docx",
      size: "840 KB",
      icon: IconClipboardList,
      color: "text-blue-500 bg-blue-500/10",
    },
    {
      name: "Lab Diagram - Circuit.png",
      size: "1.1 MB",
      icon: IconPhoto,
      color: "text-violet-500 bg-violet-500/10",
    },
    {
      name: "Reference Paper Link",
      size: "URL",
      icon: IconLink,
      ...
      <div className="rounded-2xl border border-border/40 bg-card shadow-2xl shadow-black/10 overflow-hidden">
        ...
        <div className="p-4 space-y-2">
          {files.map((f) => (
            <div
              key={f.name}
              className="flex items-center gap-3 p-3 rounded-xl border border-border/20 hover:border-primary/20 hover:bg-muted/20 transition-all cursor-pointer"
            >
              <div
                className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${f.color}`}
              >
                <f.icon size={16} />
              </div>
              <div className="flex-1 min-w-0">
  ...
  return (
    <>
      ...
      <ShowcaseSection
        ...
        bullets={[
          {
            label: "Faculty directory & profiles",
            sub: "Full visibility into every instructor and their teaching load",
          },
          {
            label: "Role-based access control",
            sub: "Admins, instructors, and students each see exactly what they need",
          },
          {
            label: "Departmental organization",
            sub: "Group faculty by department for streamlined communication",
          },
        ]}
        ...
        bullets={[
          {
            ...
            sub: "Set up a new course in seconds with a name, description, and subject",
          },
          {
            label: "Upcoming deadline tracking",
            sub: "Students and teachers always know what's due and when",
          },
          {
            label: "Classroom post streams",
            sub: "Share updates, materials, and tasks in a dedicated course feed",
          },
        ]}
      ...
      <ShowcaseSection
        ...
        bullets={[
          {
            label: "Categorized notices",
            sub: "Urgent, Event, and General tags keep the feed structured",
          },
          {
          ...
          },
          {
            ...
            sub: "Admins publish campus-wide; teachers publish to their classrooms",
          },
        ]}
        ...
        bullets={[
          {
            ...
            sub: "PDFs, documents, images, and external links all in one place",
          },
          {
          ...
          },
          {
            ...
            sub: "Every upload triggers a real-time alert to enrolled students",
          },
        ]}
