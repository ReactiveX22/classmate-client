...
// Simple direction hook replacement
function useDirection(dir?: Direction): Direction {
  const [direction, setDirection] = React.useState<Direction>(dir || 'ltr');
...
function ActionBarItem(props: ActionBarItemProps) {
  ...
  return (
    <Button
      ...
      // @ts-expect-error - Button primitive types might not include ref in legacy definitions
      ref={composedRef}
      onClick={onClick}
      onFocus={onFocus}
      onKeyDown={onKeyDown}
